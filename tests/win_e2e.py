...
def key(name, up=False):
    vk = VK[name]
    user32.keybd_event(vk, user32.MapVirtualKeyW(vk, 0) & 0xFF, 2 if up else 0, 0)
...
def run_case(name, text, trigger, check, menu_keys=(), accept_preview=False, seconds=300):
...
def main():
    ...
    for name, text, trigger, check, menu_keys, accept in cases:
        try:
            run_case(name, text, trigger, check, menu_keys, accept)
        except Exception as e:
