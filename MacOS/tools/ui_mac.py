...
from config_mac import (
    load_config, save_config, is_auto_start_enabled, set_auto_start, get_resource_path,
    get_hotkey_keycaps, get_hotkey_label
)
from hardware_mac import detect_hardware
from downloader_mac import MODELS, download_model, resolve_model
...
# Premium Native Typography (Apple San Francisco on macOS, Segoe UI on Windows)
if sys.platform == "darwin":
    PRIMARY_FONT = ".AppleSystemUIFont"
    PRIMARY_BOLD = ".AppleSystemUIFont"
    MONO_FONT = "SF Mono"
else:
    PRIMARY_FONT = "Segoe UI"
    PRIMARY_BOLD = "Segoe UI Semibold"
    MONO_FONT = "Consolas"

FONT_APP_TITLE = (PRIMARY_FONT, 16, "bold")
FONT_TITLE = (PRIMARY_FONT, 13, "bold")
FONT_HEADING = (PRIMARY_BOLD, 11)
FONT_SUBTITLE = (PRIMARY_FONT, 9)
FONT_BODY = (PRIMARY_FONT, 9)
FONT_BODY_BOLD = (PRIMARY_BOLD, 9)
FONT_SMALL = (PRIMARY_FONT, 8)
FONT_SMALL_BOLD = (PRIMARY_BOLD, 8)
FONT_MONO = (MONO_FONT, 9, "bold")
...
class DashboardWindow:
    ...
    def _build_ui(self):
        ...
        CurvedButton(
            footer,
            text="Hide to Menu Bar" if sys.platform == "darwin" else "Hide to System Tray",
        ...
        self.custom_fix_var = tk.StringVar(value=self.config.get("custom_fix", "<cmd>+<alt>+f"))
        self.custom_pol_var = tk.StringVar(value=self.config.get("custom_polish", "<cmd>+<alt>+p"))

        presets_frame = tk.Frame(pref_card, bg=SURFACE_CARD)
        presets_frame.pack(fill=tk.X, pady=(0, 2))

        presets = [
            ("double_tap", "Double-Tap Modifiers (⌥ ⌥ Fix  •  ⌃ ⌃ Polish) — Recommended"),
            ("option_space", "Option + Space (⌥ Space Fix  •  ⌥ ⇧ Space Polish)"),
            ("classic", "Classic 3-Key (⌥ ⌘ F Fix  •  ⌥ ⌘ P Polish)"),
        ...
        chk_start = tk.Checkbutton(
            pref_card,
            text="Launch Fixelect automatically at macOS login",
...
class AccessibilityGuideWindow:
    """
    Native macOS Aqua Dark Onboarding Guide for Accessibility Permissions.
    Provides a 1-click button to open macOS System Settings, instructions,
    and a real-time permission poller that auto-dismisses once permission is granted.
    """

    def __init__(self, on_granted=None):
    ...
    def show(self):
        self.root = tk.Tk()
        self.root.title("Fixelect — Permissions Required")
        ...
        cx = (sw - 540) // 2
        cy = (sh - 440) // 2
        self.root.geometry(f"540x440+{cx}+{cy}")

        apply_window_icon(self.root)
        self.root.lift()
        self.root.focus_force()

        box = tk.Frame(self.root, bg=BG_DARK, padx=28, pady=24)
        box.pack(fill=tk.BOTH, expand=True)
        ...
        tk.Label(
            box,
            text="Fixelect runs 100% locally on your Mac. To detect double-tap triggers and replace selected text across applications, macOS requires Accessibility permission.",
            font=FONT_BODY,
            fg=TEXT_MUTED,
            bg=BG_DARK,
            wraplength=480,
        ...
        # Instructions Card
        card = FluentCard(box, padx=18, pady=16)
        card.pack(fill=tk.X, pady=(0, 16))

        steps = [
            "1. Click 'Open System Settings' below.",
            "2. Under Privacy & Security > Accessibility, find Fixelect.",
        ...
        # Action Buttons
        btn_row = tk.Frame(box, bg=BG_DARK)
    ...
    def _poll_permission(self):
        ...
        try:
            ...
            if check_accessibility_permissions(prompt=False):
                self._granted = True
                self.status_badge.update_style(text="✓ Accessibility Access Granted!", bg_color="#0B261E", border_color="#125940")
                self.status_badge.configure(fg=ACCENT_GREEN)
                self.root.after(600, self._on_granted)
                return
        except Exception:
            pass

        self._after_id = self.root.after(450, self._poll_permission)
